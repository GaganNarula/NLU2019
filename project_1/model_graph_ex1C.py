...
with graph.as_default():
    ...
    saver = tf.train.Saver(var_list = {'word_embeddings':word_embeddings})
...
with tf.Session(graph=graph) as session:
    ...
    for epoch in range(NEPOCHS):
        ...
        # checkpoint model
        save_path = saver2.save(session, CHECKPT_PATH + '/model_checkpt_exC/'+'final_model.ckpt')
    ...
    with open(CHECKPT_PATH+'test_perpC.txt', 'w') as f:
